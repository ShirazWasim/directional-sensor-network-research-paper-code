function [traj_xdata traj_ydata] = GetTargInt2(traj_xdata, traj_ydata, sensor_x_pos, sensor_y_pos, deploy_times_line, times, startTime, margin)
% A function that takes in matrices for multi-segmented lines (traj_xdata)
% and multiple sensor positions (sensor_x_pos) and outputs the trajectories
% that are not intersected by the sensor
...
for index = 1:size(sensor_x_pos,1)
    ...
    if index == 1
        ...
        if isempty(traj_xdata_rem)
            ...
            % Re-initialising variable
            traj_xdata_rem = [];
            traj_ydata_rem = [];

            % Getting valid parts of trajectories
            traj_xdata_time_red = traj_xdata(:,valid_idx3); %.*valid_idx3;
            traj_ydata_time_red = traj_ydata(:,valid_idx3); %.*valid_idx3;
